...
template<class T> T SRC<T>::strcutural_recursion(AList<T> list)
{
    ...
    T val1, val2;
    if (list.getlength() == 0)  {return e;}
    if (list.getlength() == 1)  {return func_f(list);}
    else {
...
template<class T> T SRC<T>::func_f(AList<T> list)
{
    if (list.getlength() == 1)  return list.getitem(1);
    return 0;
}

template<class T> T SRC<T>::func_g(T val1, T val2)
{
    return val1 + val2;
}
