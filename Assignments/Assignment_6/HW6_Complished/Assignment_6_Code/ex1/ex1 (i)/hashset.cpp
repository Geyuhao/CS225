...
template<class T> void hashset<T>::remove_modified(T item)
{
    ...
    while (reprarray[index] != 0) { // search for the item
        if (*reprarray[index] == item) {    // if exists
            reprarray[index] = 0;   // direct delete it
            delete_id = index;      // record the deleted position
            numitems--;
        } else {
            index = (index+1)%maxsize;
        } 
    ...
    if (delete_id == -1) {
        ...
        int current = delete_id+1;
        while (reprarray[current] != 0) {
            int ideal = hashfunc(*reprarray[current])%maxsize;
            if (ideal <= current && ideal <= delete_id) { // check the ideal position of current element
                T *temp = new T;    // if it is located behind ideal position, it can be moved front
                *temp = *reprarray[current];
                reprarray[delete_id] = temp;    // move current element to the deleted position before
                reprarray[current] = 0;
                delete_id = current;
            } 
