...
public:
    ...
    void display(void);
    // ex4 modified
    void median_insertion(T item);
    void median_delete(T item);
    void median_balance(avlnode<T> *node);
private:
...
//    avlnode<T> *findswapright(avlnode<T> *pt_swap, avlnode<T> *here);
    ...
    // Modified Part for the Ex4 of assignment
    int number(avlnode<T> *node);
    T find_right_min(avlnode<T> *node);
    T find_left_max(avlnode<T> *node);
