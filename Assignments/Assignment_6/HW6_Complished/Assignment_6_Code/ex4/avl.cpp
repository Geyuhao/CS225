...
//########### functions added for ex4 ################

/*
 * number() is used to count the number of child nodes of a certain nodes
 */
template<class T> int AVL<T>::number(avlnode<T> *node)
{
    int numitem = 1;    // count itself as a node
    if (node == NULL)   return 0;
    // use recursive to count the number of nodes
    if (node->getleft() != NULL) {
        numitem += number(node->getleft());
    }
    if (node->getright() != NULL) {
        numitem += number(node->getright());
    }
    return numitem;
}

// find_right_min() is used to search for the minimum nodes in the right subtree of a certain node
template<class T> T AVL<T>::find_right_min(avlnode<T> *node)
{
    avlnode<T> *temp = new avlnode<T>;
    temp = node->getright();
    while (temp->getleft() != NULL)     // use loop to find the minimum
        temp = temp->getleft();
    return temp->getdata();
}

// find_left_max() is used to search for the maximum node in the left subtree of a certain node
template<class T> T AVL<T>::find_left_max(avlnode<T> *node)
{
    avlnode<T> *temp = new avlnode<T>;
    temp = node->getleft();
...
template<class T> void AVL<T>::median_insertion(T item)
{
    if (root == NULL) {
        insert(item);
        return;
    }
    // if left and right subtree have same amount of nodes and the inserted item is smaller than root
    if (number(root->getright()) == number(root->getleft()) && item < root->getdata()) {
        insert(item);
        median_balance(root->getleft());    // balance all nodes in left subtree
        return;
    }
    // if left subtree is higher and the item will go right
    if (number(root->getleft()) - number(root->getright()) == 1 && item > root->getdata()) {
        insert(item);
        median_balance(root->getright());
        return;
    }
    // if left and right subtree have same amount of nodes and the inserted item is larger than root
    if (number(root->getright()) == number(root->getleft()) && item > root->getdata()) {
        insert(item);   // just insert it at first
        // now the right subtree is higher and the root is no longer the M or M+
        int right_min = find_right_min(root);       // the minimum item in the right subtree now becomes the M
        int temp = root->getdata();
        remove(right_min);      // remove this node from right
        root->setdata(right_min);   // set the new root's value
        insert(temp);       // insert the previous root into the left subtree
        median_balance(root->getleft());
        median_balance(root->getright());
        // now the tree should be balanced with left higher than right
    }

    if (number(root->getleft()) - number(root->getright()) == 1 && item < root->getdata()) {
        insert(item);   // just insert at first
        // now the left subtree contains two more elements than right subtree and the median is no longer M or M+
        int left_max = find_left_max(root);
        int temp = root->getdata();
        remove(left_max);
        root->setdata(left_max);
        insert(temp);
        median_balance(root->getright());
        median_balance(root->getleft());
    }
...
template<class T> void AVL<T>::median_delete(T item)
{
    if (root == NULL)   return;
    // if left and right subtree are of same size and the item is in right subree
    if (number(root->getright()) == number(root->getleft()) && item > root->getdata()) {
        remove(item);
        median_balance(root->getright());
        return;
    }
    // if left is higher than the right and the item is in left tree
    if (number(root->getleft()) - number(root->getright()) == 1 && item < root->getdata()) {
        remove(item);
        median_balance(root->getleft());
        return;
    }
    // if left and right subtrees are of same size and the item is in left tree
    if (number(root->getright()) == number(root->getleft()) && item < root->getdata()) {
        remove(item);   // just remove it at first
        // now the right tree is higher than left tree and the root is no longer M or M+
        int right_min = find_right_min(root);
        int temp = root->getdata();
        remove(right_min);
        root->setdata(right_min);
        insert(temp);
        // same as insert, now the tree is balanced
        median_balance(root->getright());
        median_balance(root->getleft());
    }
...
template<class T> void AVL<T>::median_balance(avlnode<T> *node)
{
    if (node == NULL)   return;
    if (number(node->getright()) - number(node->getleft()) == 1)  {
        int right_min = find_right_min(node);
        int temp = node->getdata();
        remove(right_min);
        node->setdata(right_min);
        insert(temp);
    }
    // recursive to check all the nodes to see if it satisfies the median
    median_balance(node->getright());
    median_balance(node->getleft());
    return;
}
...
template<class T> avlnode<T> *AVL<T>::_insert(avlnode<T> *pt, T val)
{
    ...
    if (val < (*pt).getdata()) // the case for insertion into the left successor tree
    {
        ...
        (*pt).setleft(pt_new);
        /* the first case is the do-nothing case; the insertion into the left successor did not alter the height */
        if (mode == false)
            return pt;
        else
        {
