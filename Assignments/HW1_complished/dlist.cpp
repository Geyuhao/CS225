...
template<class T> void DList<T>::insertionsort(void)
{
    ...
    for (int i = 0; i<length; i++)
    {
        ...
        {
            if (start >= end-1){
                if (find>getitem(start+1) && find<getitem(end+1)){
                    insert(end+1,find);
                    break;
                } else if(find<=getitem(start+1)){
                    insert(start+1,find);
                    break;
                } else{
                    insert(end+1+1,find);
...
template<class T> void DList<T>::gen_rand(int length)
{
    for (int i = 0; i < length; i++)
    {
