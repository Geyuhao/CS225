 ...
 * 
 * Mainly: this file contains main program for Computing Assignment 2
 * 
 */
#include "7_helpmain.cpp"
#include <cstdlib>
#include <iostream>
...
int main()
{
    ...
    CBPlusTree* bptree_p = new CBPlusTree(PERSON);
    BTree<Person_BP> mybtree(40,smaller,equalkey,printkey);
    MultiSet_Chaining* hash_treatment =  new MultiSet_Chaining();
...
// main program starts here
{
    ...
    case 1:
        ...
        // store all person into bptree
        Person_into_BP(bptree_p, local_1);
        Person_into_BP(bptree_p, local_2);
        Person_into_BP(bptree_p, local_3);
    ...
    if (auto_op == 1) {
        ...
        for (day_offset = 0; day_offset < 29; day_offset++) {    // represent a month
            ...
            big_register(Fib_center, bptree_p, mybtree, hash_treatment, hash_med, local_1, day_reg, day_offset);   // register local1 into the center
            big_register(Fib_center, bptree_p, mybtree, hash_treatment, hash_med,local_2, day_reg, day_offset);   // register local2 into the center
            big_register(Fib_center, bptree_p, mybtree, hash_treatment, hash_med,local_3, day_reg, day_offset);   // register local3 into the center
        ...
        while (operation != 0) {
            ...
            cout << "2. Set appointments.\n";
            cout << "3. Operation for a single patient.\n";
            cout << "4. Report System Situation.\n";
            cout << "5. Go Next Day.\n";
            cout << "6. Display Central Structure (Fibonacci Heap).\n";
            cout << "7. Database System Query Operations.\n";
            cout << "0. Quit System.\n";        
            cin >> operation;
            switch (operation){
            case 0:
            ...
            case 1: {
                ...
                big_register(Fib_center, bptree_p, mybtree, hash_treatment, hash_med,local_1, temp[0], day_offset);
                big_register(Fib_center, bptree_p, mybtree, hash_treatment, hash_med,local_2, temp[1], day_offset);
                big_register(Fib_center, bptree_p, mybtree, hash_treatment, hash_med,local_3, temp[2], day_offset);
            ...
            case 2: {
                ...
                Update(waiting_q, bptree_p, WAITING);    // update status
                ...
                if (result == -1) { // patient not found
                        ...
                        for (int i = 0; i < (int)waiting_q.size(); i++) {
                            ...
                            if (temp->person->id_number == id) {
                                if (tmp == 1) {
                                    ...
                                    bptree_p->update_status(temp->person->id_number, WITHDRAW);
                                    waiting_q.pop();
                                } else {
                                    System_load("Loading");
                                    show_info(temp);
                    ...
                    case 4: {   // in withdraw_list
                        ...
                        if (withdraw_info.find(id) != NULL) {
                            ...
                            bptree_p->update_status(id, IN_HEAP);
                        }
                        break;
                    } 
                    default:
            ...
            case 7: {
                    ...
                    System_load("Database Loading");
                    cout << "\n\n------------------- DataBase System -----------------\n";
                    int search_type = 0;
                    while (search_type != 5) {  // Data base system loop
                    cout << "\033[33mChoose Query Key ued to find a person or a group of people: \033[0m"<< endl;
                    cout << "1. ID Number\n2. Medical_status\n3. Register date\n4. Treatment type\n5. Quit\n";
                        ...
                        case 1: {
                            int id_temp;
                            cout << "\033[33mEnter the ID of the people: \033[0m";
                            cin >> id_temp;
                            System_load("Searching");
                            if (bptree_p->get_block(id_temp) != NULL) {
                                show_info(bptree_p->get_block(id_temp)->Person_[0]);
                            } else {
                                cout << "\033[32m404 NOT FOUND.\033[0m\n";
                            }
                            break;
                        }
                        case 2:
                        {
                            int risk;
                            cout << "\033[33mWhich medical status do you want to search (0~3): \033[0m";
                            cin >> risk;
                            vector<Person_BP*> temp1;
                            temp1 = hash_med->retrieval(risk);
                            System_load("Searching");
                            while(temp1.size()!=0)
                            {
                                Person_BP* one_people = temp1.back();
                                temp1.pop_back();
                                print_people_BP(one_people);
                                cout<<endl;
                            }                      
                            break;
                        }
                        ...
                        {
                            int treatment_type;
                            cout << "Which treatment_type do you want to search : ";
                            cin >> treatment_type;
                            vector<Person_BP*> temp2;
                            System_load("Searching");
                            temp2 = hash_treatment->retrieval(treatment_type);
                            while(temp2.size()!=0)
                            {
                                Person_BP* one_people = temp2.back();
                                temp2.pop_back();
                                print_people_BP(one_people);
                                cout<<endl;
                            }  
        ...
        // go next day starts 
        treating(waiting_q, treating_q, treated_q, hospital_list, 5, day_offset);
        Update(treating_q, bptree_p, TREATING);
        Update(treated_q, bptree_p, TREATED);
    ...
    Generate_data(bptree_p, "data.txt");
