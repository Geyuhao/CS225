...
bool CLeafNode::check_merge(int keyindex1, int keyindex2)
{
	if (keyindex1 < 0 || keyindex1 >= m_KeyNum || keyindex2 < 0 || keyindex2 >= m_KeyNum)	return false;
	if (m_Datas[keyindex1]->get_label() != m_Datas[keyindex2]->get_label())		return false;
	double factor;
	factor = (m_Datas[keyindex1]->record_num() + m_Datas[keyindex2]->record_num())/BLOCK_SIZE;
	if (factor < 0.5)	return true;
	else	return false;
}

DataType CLeafNode::merge_blocks(int keyindex1, int keyindex2)
{
	if (m_Datas[keyindex1]->merge_block(m_Datas[keyindex2]) != NULL) {
		removeKey(keyindex2, 0);
		m_KeyValues[keyindex1] = m_Datas[keyindex1]->get_label();
		return m_Datas[keyindex1];
	} else return NULL;
}
...
void CLeafNode::insert(KeyType key, const DataType& data)
{
	...
	setData(i, data);
	setKeyNum(m_KeyNum+1);
	if (m_KeyNum > 1 && check_merge(i-1, i)) {	// check if inserted block can be merged with previous one
		merge_blocks(i-1, i);
	} return;
}
