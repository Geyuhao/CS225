...
void Person_into_BP(CBPlusTree* &bptree, queue<People*> &local_queue)
{
	for (int i = 0; i < local_queue.size(); i++) {
		...
		Block* BP_Data = new Block(PERSON, 1);
		
		individual->status = REGISTER;				// set the status of the person;
		BP_Data->insert_record(individual);			// set the block and store the block into BPtree
		bptree->insert(temp->id_number, BP_Data);	// ID is used as the primary key of B+ Tree
		//if (bptree->search(temp->id_number))	cout << "18true\n";
		local_queue.pop();
...
void CBPlusTree::update_status(KeyType key, STATUS_TYPE status)
{
	if (type == EDGE)	return;
	...
	while(itr!=NULL)
	{
		...
		{
			if (itr->getKeyValue(i) == key) {
				itr->getData(i)->Person_[0]->status = status;
			}
		}
...
DataType CBPlusTree::get_block(KeyType key)
{
...
CBPlusTree::CBPlusTree(RELATION_TYPE t){
	type = t;
	m_Root = NULL;
	m_DataHead = NULL;
}
...
vector<DataType> CBPlusTree::select_value(KeyType compareKey, COMPARE_OPERATOR compareOpeartor)
{
